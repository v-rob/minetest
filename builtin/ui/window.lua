...
function core.receive_ui_message(player, data)
	...
	local ev = {
		window = info.window._id,
		player = info.player,
		context = info.context,
	}

	if action == WINDOW_EVENT then
		info.window:_on_window_event(code, ev, data)
	elseif action == ELEM_EVENT then
		info.window:_on_elem_event(code, ev, data)
	else
