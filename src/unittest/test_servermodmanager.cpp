...
void TestServerModManager::testCreation()
{
	...
	UASSERT_EQ(world_config.updateConfigFile(path.c_str()), true);
...
void TestServerModManager::testGetModsWrongDir()
{
	...
	UASSERT_EQ(sm.getMods().empty(), true);
}

void TestServerModManager::testUnsatisfiedMods()
{
	auto sm = makeManager(m_worlddir);
	UASSERT_EQ(sm.getUnsatisfiedMods().empty(), true);
}

void TestServerModManager::testIsConsistent()
{
	auto sm = makeManager(m_worlddir);
	UASSERT_EQ(sm.isConsistent(), true);
}
...
void TestServerModManager::testGetMods()
{
	...
	for (const auto &m : mods) {
		...
		UASSERT_EQ(m.path.empty(), false);
	}

	UASSERT_EQ(default_found, true);
	UASSERT_EQ(test_mod_found, true);
...
{
	auto sm = makeManager(m_worlddir);
	UASSERT_EQ(sm.getModSpec("wrongmod"), nullptr);
	UASSERT_NE(sm.getModSpec("basenodes"), nullptr);
}
