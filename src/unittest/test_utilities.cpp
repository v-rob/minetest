...
{
	UASSERT_EQ(lowercase("Foo bAR"), "foo bar");
	UASSERT_EQ(lowercase(u8"eeeeeeaaaaaaaaaaaààààà"), u8"eeeeeeaaaaaaaaaaaààààà");
	// intentionally won't handle Unicode, regardless of locale
	UASSERT_EQ(lowercase(u8"ÜÜ"), u8"ÜÜ");
	UASSERT_EQ(lowercase("MINETEST-powa"), "minetest-powa");
}
...
void TestUtilities::testUTF8()
{
	...
	UASSERT_EQ(wide_to_utf8(L"¤"), u8"¤");

	UASSERT_EQ(wide_to_utf8(utf8_to_wide("")), "");
	UASSERT_EQ(wide_to_utf8(utf8_to_wide("the shovel dug a crumbly node!")),
		"the shovel dug a crumbly node!");

	UASSERT_EQ(wide_to_utf8(utf8_to_wide(u8"-ä-")),
		u8"-ä-");
	UASSERT_EQ(wide_to_utf8(utf8_to_wide(u8"-\U0002000b-")),
		u8"-\U0002000b-");
...
void TestUtilities::testRemoveEscapes()
{
	...
	UASSERT_EQ(unescape_enriched("abc\x1b(not this\\\\)def"),
		"abcdef");
	UASSERT(unescape_enriched<wchar_t>(
	...
	// Multiple
	UASSERT_EQ(unescape_enriched("one\x1bX two \x1b(four)three"),
		"one two three");
}
...
{
	UASSERT_EQ(sanitizeDirName("a", "~"), "a");
	UASSERT_EQ(sanitizeDirName("  ", "~"), "__");
	UASSERT_EQ(sanitizeDirName(" a ", "~"), "_a_");
	UASSERT_EQ(sanitizeDirName("COM1", "~"), "~COM1");
	UASSERT_EQ(sanitizeDirName("COM1", ":"), "_COM1");
	UASSERT_EQ(sanitizeDirName(u8"cOm\u00B2", "~"), u8"~cOm\u00B2");
	UASSERT_EQ(sanitizeDirName("cOnIn$", "~"), "~cOnIn$");
	UASSERT_EQ(sanitizeDirName(" cOnIn$ ", "~"), "_cOnIn$_");
}
...
void TestUtilities::testColorizeURL()
{
	...
	UASSERT_EQ(result, (GREY "http://" WHITE "example.com" GREY "/"));
...
void TestUtilities::testSanitizeUntrusted()
{
	...
	UASSERT_EQ(sanitize_untrusted(t1), t1);

	std::string_view t2{"stop\x00here", 9};
	UASSERT_EQ(sanitize_untrusted(t2), "stop");

	UASSERT_EQ(sanitize_untrusted("\x01\x08\x13\x1dhello\r\n\tworld"), "hello\n\tworld");

	std::string_view t3{"some \x1b(T@whatever)text\x1b" "E here"};
	UASSERT_EQ(sanitize_untrusted(t3), t3);
	...
	UASSERT_EQ(sanitize_untrusted("\x1b[31m"), "[31m");

	// edge cases
	for (bool keep : {true, false}) {
		UASSERT_EQ(sanitize_untrusted("\x1b", keep), "");
		UASSERT_EQ(sanitize_untrusted("\x1b(", keep), "(");
	}
}
...
{
	UASSERT_EQ(read_seed("123"), 123U);
	UASSERT_EQ(read_seed("0x123"), 0x123U);
	// hashing should produce some non-zero number
	UASSERT_NE(read_seed("hello"), 0U);
}
...
void TestUtilities::testGetMemorySize()
{
	...
	if (total != 0) {
		...
		UASSERT_CMP(>=, total, 130U);
		UASSERT_CMP(<, total, 8U * 1024U * 1024U);
	} else {
