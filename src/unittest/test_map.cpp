...
void TestMap::testMaxMapgenLimit()
{
	...
	UASSERT_EQ(MAX_MAP_GENERATION_LIMIT % MAP_BLOCKSIZE, MAP_BLOCKSIZE - 1);
...
void TestMap::testForEachNodeInArea(IGameDef *gamedef)
{
	...
	UASSERT_EQ(n_visited, volume_visit);
	UASSERT_EQ((s32)visited.size(), volume_visit);
	UASSERT(minp_visited == minp_visit);
	UASSERT(maxp_visited == maxp_visit);

	UASSERT_EQ(found.size(), 4U);
	UASSERT(found.find(p1) != found.end());
	UASSERT_EQ(found[p1].getContent(), n1.getContent());
	UASSERT(found.find(p2) != found.end());
	UASSERT_EQ(found[p2].getContent(), n2.getContent());
	UASSERT(found.find(p3) != found.end());
	UASSERT_EQ(found[p3].getContent(), n3.getContent());
	UASSERT(found.find(p4) != found.end());
	UASSERT_EQ(found[p4].getContent(), n4.getContent());
}
