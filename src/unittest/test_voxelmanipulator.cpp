...
void TestVoxelManipulator::testEmerge(IGameDef *gamedef)
{
	...
	UASSERT(vm.m_area.MinEdge == v3s16(0));
	UASSERT(vm.m_area.MaxEdge == v3s16(bs-1));
	UASSERT(vm.getNodeNoExNoEmerge({0,0,0}).getContent() == CONTENT_AIR);
	...
	UASSERT(vm.m_area.getExtent() == v3s32(bs,2*bs,bs));

	UASSERT(vm.getNodeNoExNoEmerge({0,   1,0}).getContent() == CONTENT_AIR);
	UASSERT(vm.getNodeNoExNoEmerge({0,bs+1,0}).getContent() == t_CONTENT_BRICK);

	// emerge out of bounds: should produce empty data
	vm.initialEmerge({0,2,0}, {0,2,0}, false);
	UASSERT(vm.m_area.getExtent() == v3s32(bs,3*bs,bs));

	UASSERT(vm.getNodeNoExNoEmerge({0,2*bs,0}).getContent() == CONTENT_IGNORE);
...
void TestVoxelManipulator::testBlitBack(IGameDef *gamedef)
{
	...
	UASSERT(modified.size() == 1);
	UASSERT(modified.begin()->first == v3s16(0,0,0));

	UASSERT(map.getNode({0,0,0}).getContent() == t_CONTENT_STONE);
	UASSERT(map.getNode({1,1,1}).getContent() == t_CONTENT_GRASS);
	// ignore nodes are not written (is this an intentional feature?)
	UASSERT(map.getNode({2,2,2}).getContent() == CONTENT_AIR);
}
...
void TestVoxelManipulator::testBlitBack2(IGameDef *gamedef)
{
	...
	UASSERT(modified.size() == 1);
	UASSERT(modified.begin()->first == v3s16(0,0,0));
	UASSERT(map.getNode({0,1,0}).getContent() == t_CONTENT_TORCH);
	// The upper one should not!
	UASSERT(map.getNode({0,bs,0}).getContent() == CONTENT_AIR);
}
