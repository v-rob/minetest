...
{
	UASSERT_EQ(np1->offset, np2->offset);
	UASSERT_EQ(np1->scale, np2->scale);
	UASSERT(np1->spread == np2->spread);
	UASSERT_EQ(np1->seed, np2->seed);
	UASSERT_EQ(np1->octaves, np2->octaves);
	UASSERT_EQ(np1->persist, np2->persist);
	UASSERT_EQ(np1->lacunarity, np2->lacunarity);
	UASSERT_EQ(np1->flags, np2->flags);
}
...
void TestMapSettingsManager::testMapMetaSaveLoad()
{
	...
	UASSERT_EQ(params1.seed, 12345U);
	UASSERT_EQ(params1.water_level, 5);
	UASSERT_EQ(params2.seed, 12345U);
	UASSERT_EQ(params2.water_level, 5);
}
...
#define GET(x) EmergeManager::getContainingChunk(x, csize)
	...
	UASSERT_EQ(GET(v3s16(-2, -2, -2)), v3s16(-2, -2, -2));
	UASSERT_EQ(GET(v3s16(0, 0, 0)), v3s16(-2, -2, -2));
	UASSERT_EQ(GET(v3s16(1, 1, 1)), v3s16(-2, -2, -2));
	UASSERT_EQ(GET(v3s16(2, 2, 2)), v3s16(-2, -2, -2));
	UASSERT_EQ(GET(v3s16(2, 3, 2)), v3s16(-2, 3, -2));
	UASSERT_EQ(GET(v3s16(0, -3, 0)), v3s16(-2, -7, -2));

	csize = v3s16(5, 2, 5);
	UASSERT_EQ(GET(v3s16(0, 0, 0)), v3s16(-2, -1, -2));
	UASSERT_EQ(GET(v3s16(0, 1, 0)), v3s16(-2, 1, -2));
	UASSERT_EQ(GET(v3s16(3, 3, 3)), v3s16(3, 3, 3));

	csize = v3s16(1);
	UASSERT_EQ(GET(v3s16(1, 2, 3)), v3s16(1, 2, 3));
	UASSERT_EQ(GET(v3s16(-3, -2, -1)), v3s16(-3, -2, -1));
