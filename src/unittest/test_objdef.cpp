...
void TestObjDef::testAddGetSetClear()
{
	...
	UASSERT(testmgr.getNumObjects() == 3);
	...
	testmgr.clear();
	UASSERT(testmgr.getNumObjects() == 0);
}
...
void TestObjDef::testClone()
{
	...
	UASSERT(mgrcopy);
	UASSERT(mgrcopy->getType() == testmgr.getType());
	UASSERT(mgrcopy->getNumObjects() == testmgr.getNumObjects());
	...
	UASSERT(obj->testvalue == temp2->testvalue);
