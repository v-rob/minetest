...
void TestVoxelAlgorithms::testVoxelLineIterator()
{
	...
	for (auto l : lines) {
		...
		while (iterator.hasNext()) {
			...
			UASSERT((new_voxel - old_voxel).getLengthSQ() == 1);
		...
		// Test node count
		UASSERT(actual_nodecount == nodecount);
	}
}
...
void TestVoxelAlgorithms::testLighting(IGameDef *gamedef)
{
	...
	const NodeDefManager *ndef = gamedef->ndef();
	{
		...
		UASSERT(n.getLight(LIGHTBANK_NIGHT, ndef->getLightingFlags(n)) == 0);
		UASSERT(n.getLight(LIGHTBANK_DAY, ndef->getLightingFlags(n)) == 13);
	}
	{
		MapNode n = map.getNode(v3s16(0, 1, 0));
		UASSERT(n.getLight(LIGHTBANK_NIGHT, ndef->getLightingFlags(n)) == 12);
		UASSERT(n.getLight(LIGHTBANK_DAY, ndef->getLightingFlags(n)) == 12);
	}
	{
		MapNode n = map.getNode(v3s16(-9, -1, 0));
		UASSERT(n.getLight(LIGHTBANK_NIGHT, ndef->getLightingFlags(n)) == 3);
		UASSERT(n.getLight(LIGHTBANK_DAY, ndef->getLightingFlags(n)) == 12);
	}
	{
		MapNode n = map.getNode(v3s16(-10, 0, 0));
		UASSERT(n.getLight(LIGHTBANK_NIGHT, ndef->getLightingFlags(n)) == 3);
		UASSERT(n.getLight(LIGHTBANK_DAY, ndef->getLightingFlags(n)) == 14);
	}
	{
		MapNode n = map.getNode(v3s16(-11, 0, 0));
		UASSERT(n.getLight(LIGHTBANK_NIGHT, ndef->getLightingFlags(n)) == 2);
		UASSERT(n.getLight(LIGHTBANK_DAY, ndef->getLightingFlags(n)) == 15);
	}
	{
		// Test that irrelevant param1 values are not clobbered.
		MapNode n = map.getNode(v3s16(-10, 1, 0));
		UASSERT(n.getParam1() == 153);
	}
}
