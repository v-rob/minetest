...
void TestRandom::testPseudoRandom()
{
	...
	for (u32 i = 0; i != 256; i++)
		UASSERT_EQ(pr.next(), expected_pseudorandom_results[i]);

	s32 state = pr.getState();
	PseudoRandom pr2(state);

	for (u32 i = 0; i != 256; i++) {
		UASSERT_EQ(pr.next(), pr2.next());
	}

	PseudoRandom pr3(0);
	UASSERT_EQ(pr3.next(), 0U);
	UASSERT_EQ(pr3.next(), 21469U);
	UASSERT_EQ(pr3.next(), 9989U);

	PseudoRandom pr4(-101);
	UASSERT_EQ(pr4.next(), 3267U);
	UASSERT_EQ(pr4.next(), 2485U);
	UASSERT_EQ(pr4.next(), 30057U);
}
...
void TestRandom::testPcgRandom()
{
	...
	for (u32 i = 0; i != 256; i++)
		UASSERT_EQ(pr.next(), expected_pcgrandom_results[i]);
