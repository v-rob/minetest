...
namespace ui
{
	...
	void Window::read(std::istream &is, bool opening)
	{
		...
		if (m_root_elem != nullptr) {
			updateElems(elem_contents, set_focus, new_focused);
		} else {
			reset();
		}
	}
	...
	v2f32 Window::getPointerPos() const
	{
		return g_manager.getPointerPos(m_type);
	}

	void Window::drawAll()
	{
		// Do nothing if we have no root element.
		if (m_root_elem == nullptr) {
			return;
		}

		// Since the elements, screen size, pixel size, or style properties
		// might have changed since the last frame, layout the element tree.
		...
		// Draw all of the layouted and updated elements.
	...
	Elem *Window::sendHoveredInput(const SDL_Event &event)
	{
		...
		for (size_t i = m_ordered_elems.size(); i > 0; i--) {
			...
			if (elem->isPointerInside() && elem->processInput(event)) {
				return elem;
			}
		}
	...
	void Window::focusNextElem(bool reverse)
	{
		...
		if (m_focused_elem != nullptr) {
			start = m_focused_elem;
		} else if (m_root_elem != nullptr) {
			start = m_root_elem;
		} else {
			return;
		}
