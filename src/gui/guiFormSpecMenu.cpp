...
v2s32 GUIFormSpecMenu::getElementPosition(const v2f32 &pos) const
{
	return v2s32((pos.X + pos_offset.X) * imgsize.X,
			(pos.Y + pos_offset.Y) * imgsize.Y);
}

v2s32 GUIFormSpecMenu::getElementSize(const v2f32 &size) const
{
	return v2s32(size.X * imgsize.X, size.Y * imgsize.Y);
}

core::recti GUIFormSpecMenu::getElementRect(const v2f32 &pos, const v2f32 &size) const
{
	v2s32 i_pos  = getElementPosition(pos);
	v2s32 i_size = getElementSize(size);
	return core::recti(i_pos, i_pos + i_size);
}

v2s32 GUIFormSpecMenu::getOldElementPosition(const v2f32 &pos) const
{
	v2f32 pos_f = (v2f32(padding.X, padding.Y) + pos_offset * spacing) +
			pos * spacing;
	return v2s32(pos_f.X, pos_f.Y);
...
void GUIFormSpecMenu::createButton(ElementSpec *elem)
{
	bool is_image = elem->has("image");
	bool is_item  = elem->has("item");
	bool style_as_item = is_item;

	if (is_item && m_client == nullptr) {
		warningstream << "Invalid use of item_image_button with no client" << std::endl;
		// Act like a normal button instead
		is_item = false;
	}

	core::recti rect;

	v2f32 pos  = elem->getVector2df("pos");
	v2f32 size = elem->getVector2df("size");

	if (!elem->getBool("old_coord")) {
		rect = getElementRect(pos, size);
	} else {
		if (is_image || is_item) {
			size.X = (size.X * spacing.X) - (spacing.X - imgsize.X);
			size.Y = (size.Y * spacing.Y) - (spacing.Y - imgsize.Y);
			rect = core::recti(getOldElementPosition(pos),
					core::dimension2d<s32>(size.X, size.Y));
		} else {
			v2s32 pos = getOldElementPosition(elem->getVector2df("pos"));
			size.X = (size.X * spacing.X) - (spacing.X - imgsize.X);
	...
	bool is_exit = elem->getBool("exit");

	FieldSpec field(
		elem->getString("name"),
		elem->getWideString("label"),
		L"",
		258 + m_fields.size(),
		is_item ? 2 : 0
	);

	field.ftype = f_Button;
	field.is_exit = is_exit;

	irr_ptr<GUIButton> e(nullptr);
	if (is_image) {
		e = std::move(irr_ptr<GUIButton>(GUIButtonImage::addButton(Environment, rect,
				m_tsrc, m_current_parent, field.fid, field.flabel.c_str())));
		e->setScaleImage(true);
	} else if (is_item) {
		std::string item_name = elem->getString("item");
		e = std::move(irr_ptr<GUIButton>(GUIButtonItemImage::addButton(Environment,
				rect, m_tsrc, m_current_parent, field.fid, field.flabel.c_str(),
				item_name, m_client)));
		...
		m_tooltips[item_name] = TooltipSpec(
				utf8_to_wide(item.getDefinition(idef).description),
				m_default_tooltip_bgcolor, m_default_tooltip_color);
	} else {
		e = std::move(irr_ptr<GUIButton>(GUIButton::addButton(Environment, rect,
				m_tsrc, m_current_parent, field.fid, field.flabel.c_str())));
	}
	e->grab();

	if (elem->getBool("focused"))
	...
	auto style = getStyleForElement(type, field.fname, inherit);

	// Override style properties with values specified directly in the element
	if (is_image) {
		style.at(StyleStateSpec::DEFAULT).setString("fgimg", elem->getString("image"));

		if (elem->has("noclip"))
			style.at(StyleStateSpec::DEFAULT).setBool("noclip",
					elem->getBool("noclip"));
		if (elem->has("border"))
			style.at(StyleStateSpec::DEFAULT).setBool("border",
					elem->getBool("border", true));

		if (elem->has("pressed_image"))
			style.at(StyleStateSpec::PRESSED).setString("fgimg",
					elem->getString("pressed_image"));
	}
...
void GUIFormSpecMenu::parseSize(ParserState* data, const std::string &element)
{
	...
	if (((parts.size() == 2) || parts.size() == 3) ||
		((parts.size() > 3) && (data->formspec_version > FORMSPEC_API_VERSION)))
	{
...
void GUIFormSpecMenu::parseContainer(ParserState* data, const std::string &element)
{
	std::vector<std::string> parts = split(element, ',');
...
void GUIFormSpecMenu::parseContainerEnd(ParserState* data)
{
...
void GUIFormSpecMenu::parseScrollContainer(ParserState *data, const std::string &element)
{
	std::vector<std::string> parts = split(element, ';');

	if (parts.size() < 4 ||
			(parts.size() > 5 && data->formspec_version <= FORMSPEC_API_VERSION)) {
...
void GUIFormSpecMenu::parseList(ParserState *data, const std::string &element)
{
	if (m_client == 0) {
...
void GUIFormSpecMenu::parseCheckbox(ParserState* data, const std::string &element)
{
	...
	if (((parts.size() >= 3) && (parts.size() <= 4)) ||
		((parts.size() > 4) && (data->formspec_version > FORMSPEC_API_VERSION)))
	{
		std::vector<std::string> v_pos = split(parts[0],',');
		...
		e->setNotClipped(style.getBool("noclip"));
...
void GUIFormSpecMenu::parseImage(ParserState* data, const std::string &element)
{
		...
		((parts.size() > 3) && (data->formspec_version > FORMSPEC_API_VERSION)))
	{
		...
		e->setNotClipped(style.getBool("noclip", data->formspec_version < 3));
		m_fields.push_back(spec);
...
void GUIFormSpecMenu::parseAnimatedImage(ParserState *data, const std::string &element)
{
	...
	if (parts.size() != 6 && parts.size() != 7 &&
			!(parts.size() > 7 && data->formspec_version > FORMSPEC_API_VERSION)) {
...
void GUIFormSpecMenu::parseTable(ParserState* data, const std::string &element)
{
		...
		((parts.size() > 5) && (data->formspec_version > FORMSPEC_API_VERSION)))
	{
		...
		e->setNotClipped(style.getBool("noclip"));
		e->setOverrideFont(style.createFont("font", "font_size"));
...
void GUIFormSpecMenu::parseTextList(ParserState* data, const std::string &element)
{
	...
	if (((parts.size() == 4) || (parts.size() == 5) || (parts.size() == 6)) ||
		((parts.size() > 6) && (data->formspec_version > FORMSPEC_API_VERSION)))
	{
		std::vector<std::string> v_pos = split(parts[0],',');
		std::vector<std::string> v_geom = split(parts[1],',');
		std::string name = parts[2];
...
void GUIFormSpecMenu::parseFieldCloseOnEnter(ParserState *data, const std::string &element)
{
	std::vector<std::string> parts = split(element,';');
	if (parts.size() == 2 ||
			(parts.size() > 2 && data->formspec_version > FORMSPEC_API_VERSION)) {
...
void GUIFormSpecMenu::parsePwdField(ParserState* data, const std::string &element)
{
		...
		(parts.size() > 4 && data->formspec_version > FORMSPEC_API_VERSION))
	{
		...
		e->setNotClipped(style.getBool("noclip"));
		e->setDrawBorder(style.getBool("border", true));
		e->setOverrideColor(style.getColor("textcolor", video::SColor(0xFFFFFFFF)));
		e->setOverrideFont(style.createFont("font", "font_size"));
	...
	core::rect<s32> &rect, bool is_multiline)
{
	...
	if (e) {
		if (is_editable && spec.fname == data->focused_element)
			Environment->setFocus(e);
		...
		e->setDrawBorder(style.getBool("border", true));
		e->setOverrideColor(style.getColor("textcolor", video::SColor(0xFFFFFFFF)));
		e->setOverrideFont(style.createFont("font", "font_family"));
	...
	if (!spec.flabel.empty()) {
		...
		if (t)
			t->setNotClipped(style.getBool("noclip"));
	}
}
...
void GUIFormSpecMenu::parseHyperText(ParserState *data, const std::string &element)
{
	...
	if (parts.size() != 4 && data->formspec_version < FORMSPEC_API_VERSION) {
...
void GUIFormSpecMenu::parseLabel(ParserState* data, const std::string &element)
{
		...
		((parts.size() > 2) && (data->formspec_version > FORMSPEC_API_VERSION)))
	{
		...
		gui::IGUIFont *font = style.createFont("font", "font_size", m_font);
...
void GUIFormSpecMenu::parseTabHeader(ParserState* data, const std::string &element)
{
	...
	if (((parts.size() == 4) || (parts.size() == 6)) || (parts.size() == 7 &&
		...
		(data->formspec_version > FORMSPEC_API_VERSION)))
	{
		std::vector<std::string> v_pos = split(parts[0],',');
		...
		for (const std::string &button : buttons) {
			...
			if (style.has("bgcolor"))
				tab->setBackgroundColor(style.getColor("bgcolor"));

			tab->setTextColor(style.getColor("textcolor", video::SColor(0xFFFFFFFF)));
		}
...
void GUIFormSpecMenu::parseBox(ParserState* data, const std::string &element)
{
		...
		((parts.size() > 3) && (data->formspec_version > FORMSPEC_API_VERSION)))
	{
		...
		core::recti borderwidths(0, 0, 0, 0);

		if (parseColorString(parts[2], tmp_color, true, 0x8C)) {
			colors = {tmp_color, tmp_color, tmp_color, tmp_color};
		} else {
			colors = style.getColorArray("colors");
			bordercolors = style.getColorArray("bordercolors");
			borderwidths = style.getRecti("borderwidths");
		}
...
void GUIFormSpecMenu::parseBackgroundColor(ParserState* data, const std::string &element)
{
	...
	if ((parameter_count > 2 && data->formspec_version < 3) ||
			(parameter_count > 3 && data->formspec_version <= FORMSPEC_API_VERSION)) {
	...
	if (parameter_count >= 2) {
		...
		} else if (parts[1] != "" || data->formspec_version < 3) {
...
bool GUIFormSpecMenu::parseVersionDirect(ParserState *state, const std::string &data)
{
	//some prechecks
	if (data.empty())
		return false;
	...
	if (is_number(parts[1])) {
		state->formspec_version = mystoi(parts[1]);
		return true;
	}
...
bool GUIFormSpecMenu::parseStyle(ParserState *data, const std::string &element, bool style_type)
{
	...
	for (size_t sel = 0; sel < selectors.size(); sel++) {
		...
		std::vector<StyleStateSpec::State> states;
		...
		if (state_pos != std::string::npos) {
			...
			if (state_str.empty()) {
				...
				std::vector<std::string> v_states = split(state_str, '+');
				for (size_t i = 0; i < states.size(); i++) {
					if (v_states[i] == "default") {
						states.push_back(StateStyleSpec::DEFAULT);
					} else if (v_states[i] == "hovered") {
						states.push_back(StateStyleSpec::HOVERED);
					} else if (v_states[i] == "pressed") {
						states.push_back(StateStyleSpec::PRESSED);
					} else P
						warningstream << "Unknown style state " << state <<
		...
		if (style_type) {
			for (size_t i = 0; i < states.size(); i++)
				theme_by_type[selector].at(states[i]) = selector;
		} else {
			for (size_t i = 0; i < states.size(); i++)
				theme_by_type[selector].at(states[i]) = selector;
		}
...
void GUIFormSpecMenu::parseSetFocus(ParserState *state, const std::string &element)
{
	std::vector<std::string> parts = split(element, ';');

	if (parts.size() <= 2 ||
		(parts.size() > 2 && state->formspec_version > FORMSPEC_API_VERSION))
	{
...
void GUIFormSpecMenu::parseElement(ParserState* data, const std::string &element)
{
	...
	std::unique_ptr<ElementSpec> elem = parser.parseElement(data, element);
	if (elem != nullptr) {
#define CREATOR(f) &GUIFormSpecMenu::create##f
		...
		(this->*creators[elem->getElementType()])(elem.get());
		return;
	}
#undef CREATOR
	...
	if (parseVersionDirect(data, element))
		return;
	...
	if (type == "set_focus") {
		parseSetFocus(data, description);
		return;
	}
...
void GUIFormSpecMenu::regenerateGui(v2u32 screensize)
{
			...
			if (focused_id >= 258) {
				for (const GUIFormSpecMenu::FieldSpec &field : m_fields) {
					if (field.fid == focused_id) {
						mydata.focused_element = field.fname;
						break;
					}
				}
			}
	...
	std::cout << "DEBUG: FORMSPEC STRING\n" << m_formspec_string << "\nEND FORMSPEC STRING" << std::endl;
	std::vector<std::string> elements = split(m_formspec_string,']');
	unsigned int i = 0;

	/* try to read version from first element only */
	if (!elements.empty()) {
		if (parseVersionDirect(&mydata, elements[0])) {
			i++;
		}
	}
	...
	mydata.real_coordinates = mydata.formspec_version >= 2;
	for (; i < elements.size(); i++) {
		std::vector<std::string> parts = split(elements[i], '[');
	...
	if (enable_prepends) {
		...
		u16 version_backup = mydata.formspec_version;
		...
		mydata.formspec_version = version_backup;
...
{
	return getStyleForElement(type, name, parent_type)[StyleStateSpec::DEFAULT];
}

StyleStateSpec GUIFormSpecMenu::getStyleForElement(const std::string &type,
		const std::string &name, const std::string &parent_type)
{
	StyleStateSpec ret;
